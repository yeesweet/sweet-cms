...
(function($){
    ...
    $.fn.yxMobileSlider = function(settings){
        ...
        settings = $.extend(true, {}, defaultSettings, settings);         
            ...
            return own.each(function(index){
            var _this = $(this), s = settings;
            var startX = 0, startY = 0; //触摸开始时手势横纵坐标 
            var temPos; //滚动元素当前位置
            var iCurr = 0; //当前滚动屏幕数
            var timer = null; //计时器
            var oMover = $("dl", _this); //滚动元素			
            var oLi = $("dd", oMover); //滚动单元
            ...
            //初始化焦点容器及按钮	
           // $(".a1 focus").eq(0).show().siblings().hide();		
            _this.append("<div id='focus"+index+"'><div></div></div>");
            var oFocusContainer = $("#focus"+index);
			
			index_++;
		   //$("#focus"+index+"").show().siblings(".focus").hide();
            for (var i = 0; i < num; i++) {
