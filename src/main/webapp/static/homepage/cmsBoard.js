...
$(function(){ 
	...
	for(var i=1;i<=num;i++){
	    ...
	    if(data==101){//轮播图
	    	divHtml=divHtml+"<p class='time'><span class='red'>*</span>起止时间：<input type='text' class='ipt_text' readonly='readonly' name='boardDetails["+(i-1)+"].subStartTime' id='subStartTime"+i+"' " +
	    		"class='Wdate' onFocus='WdatePicker({dateFmt:\"yyyy-MM-dd HH:mm:ss\",readOnly:true})' size='20'/>&nbsp;&nbsp;"+
				"至  &nbsp;&nbsp;<input type='text' class='ipt_text' readonly='readonly'  name='boardDetails["+(i-1)+"].subEndTime' id='subEndTime"+i+"' class='Wdate'" +
	    ...
	    if(!(data==16 || data==20)){
	    	...
	    	"<input type='hidden' id='img"+i+"' name='boardDetails["+(i-1)+"].imgurl'/>"+
	    	"</div>";
	    }
	    if(data==102 || data==17){//推荐功能模块和模块17和推荐功能模块二
	    	divHtml=divHtml+"<p class='title'><span class='red'>&nbsp;</span>模块名称：<input type='text' id='title"+i+"' name='boardDetails["+(i-1)+"].title' class='ipt_text'/></p>";
	    }
	    if(data==104){//优购快报
	    	divHtml=divHtml+"<p class='title'><span class='red'>*</span>快报标题：<input type='text' id='title"+i+"' name='boardDetails["+(i-1)+"].title' class='ipt_text' style='width:300px;'/>(50个字以内)</p>";
	    }
	    if(data == 16){
	    ...
	    if(!(data==16 || data == 18)){
	    	if(currentPLC == "vm8"){
	    		divHtml=divHtml+"<option value='100'>横滑专题列表</option>"+
	    		"<option value='1025'>竖滑专题列表</option>"+
	    		"<option value='13'>web专题列表</option>"+
	    		"<option value='1028'>账户管理</option>"+
	    		"<option value='1001'>摇一摇页面</option>";
	    	}
	...
	/*    	"<option value='1006'>分类二级页面</option>"+*/
	    	...
	    	if(currentPLC == "wap"){
	    		divHtml=divHtml+"<option value='1023'>会员中心</option>"+
	    		"<option value='22'>我的优购</option>"+
	    		"<option value='1014'>我的订单</option>"+
	    		"<option value='1015'>我的收藏</option>"+
	    		"<option value='1024'>频道页</option>";
	    	}
	    	divHtml=divHtml+"<option value='2'>分类列表</option>"+
	    	"<option value='1'>品牌列表</option>"+
	    	"<option value='7'>搜索关键字</option>"+
	    	"<option value='1026'>页面列表</option>"+
	    	"<option value='1029'>关键字列表</option>"+
	    	"<option value='2000'>自定义参数</option>";
	    }
	    divHtml=divHtml+"</select></p>"+
	    		...
	    		"<p id='tip1024"+i+"' class='choosep' style='display:none;'><span class='red'>*</span>频&nbsp;道&nbsp;&nbsp;页：<span></span></p>"+
	    ...
	    if(data == 101||data==104){//轮播图或者优购快报
	    	divHtml=divHtml+"<span class='red'>*</span>排&nbsp;序&nbsp;&nbsp;号：<input type='text' class='ipt_text' name='boardDetails["+(i-1)+"].sortNo' id='sortNo"+i+"' value='' />";
	    }else{
	    	divHtml=divHtml+"<input type='hidden' name='boardDetails["+(i-1)+"].sortNo' id='sortNo"+i+"' value='"+i+"' />";
	    }
	    divHtml=divHtml+"<input type='hidden' name='boardDetails["+(i-1)+"].commonId' id='commonId"+i+"' />"+
				"<input type='hidden' name='boardDetails["+(i-1)+"].subtype' id='subtype"+i+"' />"+
				"<input type='hidden' name='boardDetails["+(i-1)+"].linkurl' id='linkurl"+i+"' />"+
		 		"</div>";
	    if(data == 17){//复用模块
	    	divHtml=divHtml +"</div>";
	    }
