...
<p id="tip1026${cmsModuleDetail_index+1}" class="choosep" <c:if test="${not empty cmsModuleDetail.subType && !(cmsModuleDetail.subType eq null) && cmsModuleDetail.subType != 1026}">style="display:none;"</c:if>>
	...
	<span>
		<%! int cmsActivityListFlag=0; %>
		<c:if test="${not empty cmsActivityList && !(cmsActivityList eq null)}">
			<c:forEach  items="${cmsActivityList}" var="cmsActivity">
				<c:if test="${not empty cmsModuleDetail.commonId && !(cmsModuleDetail.commonId eq null) && cmsModuleDetail.commonId?? && cmsModuleDetail.commonId==cmsActivity.id}">
					<% cmsActivityListFlag++; %>
